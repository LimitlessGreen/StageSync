// Package decode provides CGo-free audio file decoding to f32 interleaved PCM.
// It is intentionally free of any native/CGo dependencies so it can be used by
// both the embedded library (corelib / peaks) and the audio engine (audioengine).
package decode
...
// PCM decodes the audio file at path to f32 interleaved PCM resampled to
// targetRate / targetChannels. Supported: WAV, MP3, FLAC, OGG, AIFF.
func PCM(path string, targetRate, targetChannels uint32) ([]float32, uint32, uint32, error) {
...
func decodeMP3(path string, targetRate, targetChannels uint32) ([]float32, uint32, uint32, error) {
	...
	srcRate := uint32(dec.SampleRate())
	const srcChannels = 2 // go-mp3 always outputs stereo int16 LE
