...
4. **Loudness-Metadaten**: `declaredDurationMs` lässt sich aus dem Datei-Header lesen; `loudnessLufs` braucht ein echtes Audio-Analyse-Tool (z.B. `ffprobe` serverseitig beim Upload). Klären, ob serverseitige Analyse beim Upload gewünscht ist oder ob der Operator diese Werte manuell einträgt.
5. **`patched`-Status-Reporting**: Der Server berechnet `patched` und propagiert ihn im `ShowDefinitionEvent`-Snapshot. Klären, in welchem Intervall bzw. bei welchen Trigger-Events der Server diesen Status neu berechnet (bei jeder Patch-Änderung? bei jedem Node-Online/Offline? beides).
