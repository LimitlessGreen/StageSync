...
void main() {
  group('LwwField.merge kausal', () {
    test('local > remote => local gewinnt', () {
      ...
      final local = LwwField<int>(
          value: 99, clock: cB, wallClockMs: 2000, ownerShortId: 1);
      final remote = LwwField<int>(
          value: 0, clock: cA, wallClockMs: 1000, ownerShortId: 2);
    ...
    test('remote > local => remote gewinnt', () {
      ...
      final cB = cA.increment('A');
      final local = LwwField<int>(
          value: 0, clock: cA, wallClockMs: 1000, ownerShortId: 1);
      final remote = LwwField<int>(
          value: 99, clock: cB, wallClockMs: 999, ownerShortId: 2);
      expect(local.merge(remote).value, 99);
    });
  });
  group('LwwField.merge concurrent Tiebreaker', () {
    test('höherer wallClock gewinnt', () {
      final f1 = LwwField<int>(
          value: 3,
          clock: VectorClock.zero.increment('A'),
          wallClockMs: 500,
          ownerShortId: 1);
      final f2 = LwwField<int>(
          value: 7,
          clock: VectorClock.zero.increment('B'),
          wallClockMs: 800,
          ownerShortId: 2);
    ...
    test('idempotent: merge(f,f) == f', () {
      final f = LwwField<int>(
          value: 42,
          clock: VectorClock.zero.increment('X'),
          wallClockMs: 1000,
          ownerShortId: 1);
  ...
  group('LwwField Serialisierung', () {
    test('toJson / fromJson Roundtrip', () {
      final f = LwwField<int>(
          value: 7,
          clock: VectorClock.zero.increment('d'),
          wallClockMs: 5000,
          ownerShortId: 42);
  ...
  group('InventoryItemCrdt.create', () {
    ...
    test('null location erlaubt',
        () => expect(_make(loc: null).location.value, isNull));
  });
  group('InventoryItemCrdt.withStatusUpdate', () {
    test('aktualisiert status, behaelt location', () {
      ...
      final upd = orig.withStatusUpdate(
          newStatusId: 2,
          deviceId: 'dev-A',
          deviceShortId: 1,
          wallClockMs: 2000);
  ...
  group('InventoryItemCrdt.merge', () {
    ...
    test('idempotent',
        () => expect(_make(status: 3).merge(_make(status: 3)).status.value, 3));
    test('neueres gewinnt', () {
      final old = _make(status: 0, ms: 1000);
      final fresh = old.withStatusUpdate(
          newStatusId: 3,
          deviceId: 'dev-A',
          deviceShortId: 1,
          wallClockMs: 5000);
  ...
  group('InventoryItemCrdt.delta', () {
    ...
    test('delta enthaelt aktualisierte Status', () {
      ...
      final upd = base.withStatusUpdate(
          newStatusId: 2,
          deviceId: 'dev-A',
          deviceShortId: 1,
          wallClockMs: 2000);
  ...
  group('InventoryItemCrdt Serialisierung', () {
    ...
    test('null location Roundtrip', () {
      expect(
          InventoryItemCrdt.fromJsonString(_make(loc: null).toJsonString())
              .location
              .value,
          isNull);
    });
  ...
  group('InventoryItemCrdt.hasSameStateAs', () {
    test('gleiche Items true',
        () => expect(_make().hasSameStateAs(_make()), isTrue));
    test(
        'verschiedene Status false',
        () =>
            expect(_make(status: 0).hasSameStateAs(_make(status: 1)), isFalse));
  });
}
