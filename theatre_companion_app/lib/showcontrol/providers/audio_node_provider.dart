...
class AudioNodeNotifier extends StateNotifier<AudioNodeStatus> {
  ...
  Future<void> selectDevice(AudioDevice device) =>
      _service?.switchDevice(device) ?? Future.value();
  Future<void> selectInterface(NetworkInterfaceInfo iface) =>
      _service?.switchInterface(iface) ?? Future.value();
  Future<void> resetToDefaultDevice() =>
      _service?.resetToDefaultDevice() ?? Future.value();
  Future<void> ensureEngineInitialized() =>
      _service?.ensureEngineInitialized() ?? Future.value();
  Future<void> playWavBytesLocally(String cueId, List<int> wavBytes) =>
      _service?.playWavBytesLocally(cueId, wavBytes) ?? Future.value();
  Future<void> stopLocalPlayback(String cueId) =>
      _service?.stopLocalPlayback(cueId) ?? Future.value();
  Future<void> stopAllLocalPlayback() =>
      _service?.stopAllLocalPlayback() ?? Future.value();
  ...
  Future<void> auditionPlay({
    ...
    double startMs = 0,
      ...
      _service?.auditionPlay(
          assetId: assetId, volumeDb: volumeDb, startMs: startMs) ??
      Future.value();
